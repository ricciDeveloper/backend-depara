...
class ExcelService {
  ...
  parseExcelData(fileData) {
    try {
      ...
      const deRows = XLSX.utils.sheet_to_json(workbook.Sheets['DE']);
      const rastRows = XLSX.utils.sheet_to_json(workbook.Sheets['RASTREIO']);
  ...
  generateOutputExcel(results, minScore = 0.8) {
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, 'DE_x_PARA_Resultado');
